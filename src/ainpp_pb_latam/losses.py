...
class MotionFieldConservationLoss(nn.Module):
    def __init__(self, base_loss: nn.Module, beta: float = 0.1):
        super().__init__()
    ...
    def forward(self, pred, target):
        precip, motion_field = pred   # <- desempacota a tupla vinda do MFUNetForecaster
        mse_term = self.base_loss(precip, target)
        mf_flat = motion_field.reshape(-1, 2, *motion_field.shape[-2:])
        diff_u = F.conv2d(mf_flat[:, 0:1], self.sobel_x)
        diff_v = F.conv2d(mf_flat[:, 1:2], self.sobel_y)
        physics_term = torch.abs(diff_u + diff_v).mean()
        return (1 - self.beta) * mse_term + self.beta * physics_term