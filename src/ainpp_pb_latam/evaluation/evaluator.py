    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(
                tqdm(self.loader, desc="Evaluating Lead Times & Thresholds")
            ):
                ...
                for t in range(T):
                    ...
                    for thresh in self.thresholds:
                        if self.compute_cat:
                            cat_metrics = CategoricalMetrics.compute(
                                pred_t, target_t, threshold=thresh
                            )
                        ...
                        if self.compute_prob:
                            prob_metrics = ProbabilisticMetrics.compute(
                                pred_t, target_t, threshold=thresh
                            )
                        ...
                        if self.compute_obj:
                            obj_metrics = ObjectBasedMetrics.compute(
                                pred_t, target_t, threshold=thresh
                            )
